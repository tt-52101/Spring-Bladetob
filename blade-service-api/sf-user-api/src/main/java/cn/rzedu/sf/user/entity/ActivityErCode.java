package cn.rzedu.sf.user.entity;

import com.baomidou.mybatisplus.annotation.TableLogic;
import com.baomidou.mybatisplus.annotation.TableName;
import com.baomidou.mybatisplus.annotation.IdType;
import com.baomidou.mybatisplus.annotation.TableId;

import java.time.LocalDateTime;
import java.io.Serializable;

import lombok.Data;
import lombok.EqualsAndHashCode;
import io.swagger.annotations.ApiModel;
import io.swagger.annotations.ApiModelProperty;

/**
 * 活动二维码实体类
 *
 * @author Blade
 * @since 2020-09-15
 */
@Data
@TableName("sf_activity_er_code")
@ApiModel(value = "ActivityErCode对象", description = "活动二维码")
public class ActivityErCode implements Serializable {

    private static final long serialVersionUID = 1L;

    @TableId(value = "id", type = IdType.AUTO)
    private Integer id;
    /**
     * 二维码类型  1=活动海报二维码 2=推文二维码 3=课本二维码
     */
    @ApiModelProperty(value = "二维码类型  1=活动海报二维码 2=推文二维码 3=课本二维码")
    private Integer type;
    /**
     * 奖品类型  1=礼品A 2=礼品B
     */
    @ApiModelProperty(value = "奖品类型  1=礼品A 2=礼品B")
    private Integer awardType;
    /**
     * 用户id (发起人)
     */
    @ApiModelProperty(value = "用户id (发起人)")
    private Integer userId;
    /**
     * 用户uuid (发起人)
     */
    @ApiModelProperty(value = "用户uuid (发起人)")
    private String userUuid;
    /**
     * 二维码名称
     */
    @ApiModelProperty(value = "二维码名称")
    private String erName;
    /**
     * 二维码
     */
    @ApiModelProperty(value = "二维码")
    private String erCode;
    /**
     * 扫码次数
     */
    @ApiModelProperty(value = "扫码次数")
    private Integer scanCount;
    /**
     * 扫码人数
     */
    @ApiModelProperty(value = "扫码人数")
    private Integer scanPeopleCount;
    /**
     * 成功助力人数
     */
    @ApiModelProperty(value = "成功助力人数")
    private Integer assistCount;
    /**
     * 邀请系数
     */
    @ApiModelProperty(value = "邀请系数")
    private Integer inviteCoef;
    /**
     * 创建时间
     */
    @ApiModelProperty(value = "创建时间")
    private LocalDateTime createDate;
    /**
     * 修改时间
     */
    @ApiModelProperty(value = "修改时间")
    private LocalDateTime modifyDate;
    /**
     * 删除标识
     */
    @TableLogic
    @ApiModelProperty(value = "删除标识")
    private Integer isDeleted;


}
