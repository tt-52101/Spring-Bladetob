...
//			EntityFile entityFile = entityFileClient.uploadImage(file1);
			EntityFile entityFile = entityFileClient.upload(file1);
...
//							saveAsFileWriter(name + "***************************************************************" + parentName);
						...
						if(name.contains("txt")){
							...
							saveAsFileWriter(content + " : " + !"".equals(content));
							if(!"".equals(content)){
...
//	private static String[] gradeIds = {"1109", "7068", "7069", "7070", "7071", "7072", "7073", "7074"};
	private static String[] gradeIds = {"7931156", "7931157", "7931158", "7931159", "7931160", "7931161"};
	...
	private void importTxt2(String code, String font) {
		Integer i = 0;
		for (String gradeId : gradeIds) {
			if ("7931156".equals(gradeId + "") || "7931157".equals(gradeId + "")) {
				font = "欧体";
				continue;
			} else {
				font = "字体1";
...
//						}
	...
	private Integer getResourceTypeByCode(String code) {
		...
		if ("white_character".equals(code)) {
			...
			type = 711;
		} else if ("matts".equals(code) || "evolve_image".equals(code) || "usage_audio".equals(code) || "usage_text".equals(code)) {
			type = 712;
		} else if ("observe_image".equals(code) || "observe_dot".equals(code) || "observe_arrow".equals(code) || "observe_triangle".equals(code)) {
			type = 713;
		} else if ("analyse_image".equals(code)) {
			type = 714;
		} else if ("stroke_text".equals(code) || "stroke_audio".equals(code)) {
			type = 714;
		} else if ("space_text".equals(code) || "space_audio".equals(code)) {
			type = 714;
		} else if ("technique_line".equals(code)) {
			type = 715;
		} else if ("technique_gesture".equals(code)) {
			type = 715;
		} else if ("learn_text".equals(code) || "learn_video".equals(code)) {
			type = 716;
		} else if ("compare_text".equals(code) || "compare_image".equals(code)) {
			type = 717;
	...
	private EntityFile uploadFile(File file, String code) throws IOException {
		...
		if ("technique_line".equals(code) || "technique_gesture".equals(code) || "learn_video".equals(code)
				|| "usage_audio".equals(code) || "stroke_audio".equals(code) || "space_audio".equals(code)) {
